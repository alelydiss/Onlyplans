        ...
        @else
            ...
            <div id="cardsView" class="hidden">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    @foreach ($usuarios as $usuario)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-5 flex flex-col items-center user-row"
                             data-name="{{ strtolower($usuario->name.' '.$usuario->last_name) }}"
                             data-email="{{ strtolower($usuario->email) }}">
                            <div class="mb-3">
                                @if($usuario->avatar)
                                    <img class="h-16 w-16 rounded-full object-cover shadow" src="{{ asset('storage/'.$usuario->avatar) }}" alt="{{ $usuario->name }}">
                                @else
                                    <div class="h-16 w-16 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center shadow">
                            ...
                            <div class="text-center">
                                <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ $usuario->name }} {{ $usuario->last_name }}</div>
                                <div class="text-sm text-gray-500 dark:text-gray-400 mb-2">{{ $usuario->email }}</div>
