<nav class="bg-[#1f1a38] dark:bg-gray-800 text-white shadow-md" x-data="{ open: false }">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    @if (Auth::user()->role === 'admin')
                        ...
                        <a href="{{ route('admin.dashboard') }}"
                            class="text-sm font-medium hover:text-purple-400 {{ request()->routeIs('admin.dashboard') ? 'text-purple-400' : '' }}">Inicio</a>
                        <a href="{{ route('admin.categorias') }}"
                            class="text-sm font-medium hover:text-purple-400 {{ request()->routeIs('admin.categorias') ? 'text-purple-400' : '' }}">Categorías</a>
                        <a href="{{ route('admin.usuarios') }}"
                            class="text-sm font-medium hover:text-purple-400 {{ request()->routeIs('admin.usuarios') ? 'text-purple-400' : '' }}">Usuarios</a>
                        <a href="{{ route('admin.eventos') }}"
                            class="text-sm font-medium hover:text-purple-400 {{ request()->routeIs('admin.eventos') ? 'text-purple-400' : '' }}">Eventos</a>
                    @else
    ...
    @auth
        <div x-show="open"
            ...
            @if (Auth::user()->role === 'admin')
                <a href="{{ route('admin.dashboard') }}" class="block hover:text-purple-400">Inicio</a>
                <a href="{{ route('admin.categorias') }}" class="block hover:text-purple-400">Categorías</a>
                <a href="{{ route('admin.usuarios') }}" class="block hover:text-purple-400">Usuarios</a>
                <a href="{{ route('admin.eventos') }}" class="block hover:text-purple-400">Eventos</a>
            @else
                <a href="{{ route('dashboard') }}" class="block hover:text-purple-400">Inicio</a>
                <a href="{{ route('eventosPersonalizados') }}" class="block hover:text-purple-400">Planes</a>
                <a href="{{ route('mapa') }}" class="block hover:text-purple-400">Mapa</a>
                <a href="#" class="flex items-center hover:text-purple-400"><i class="bi bi-stars"></i><span>Favoritos</span></a>
                <a href="#" class="flex items-center hover:text-purple-400"><i class="bi bi-ticket-fill"></i><span>Tickets</span></a>
            @endif
